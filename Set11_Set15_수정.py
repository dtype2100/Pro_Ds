...
# 1. 3년 연속 행복지수가 기록된 국가의 데이터를 사용 => 국가별 기록 수 확인
# DF.apply(함수명 또는 lambda ~ 함수제작 가능)

q1=data11.groupby('Country').apply(len)

sel_list=q1[q1 == 3].index

# 2. 3년 연속 데이터가 기록되지 않은 국가의 개수
(q1 < 3).sum()
...
tab=data11.pivot_table(index='Country',
                       columns='year',
                       values='Happiness_Score')

# 결측치가 포함되었다는 의미는 즉 3년 연속 기록이 없다는 의미
tab.isna().any(axis=1).sum()

# 3년 연속 데이터
q2=tab.dropna()

# 증감률 = (2017년행복지수−2015년행복지수)/2 변수 생성
# 국가별로 생성해야 함, 국가별로 필터링해서 연도별 매핑
...
from scipy.stats import f_oneway  # 분산분석 : 집단 간의 평균 차이 유무
from statsmodels.formula.api import ols
from statsmodels.stats.anova import anova_lm # 집단 간의 평균 차이 유무
...
# [참고]
# 다중비교/사후 검정 : 귀무가설 기각 시 사용, 평균 차이나는 그룹 존재 시 사용
...
# 1. 가장 많은 제품이 팔린 날짜에
# - 날짜 빈도를 이용해 일자별 판매 수 도출
search_date=pos1['Date'].value_counts().idxmax()

# 2. 찾은 날짜에 가장 많이 팔린 제품의 판매 개수

pos1[pos1.Date==search_date]['itemDescription'].value_counts().nlargest(1)
...
# 1. 변수 생성: 요일, 분기/월, 금토유무

pos1['Date'].str.split('-')

pos2=pos1.copy()
pos2['month']=pd.to_datetime(pos1['Date']).dt.month
pos2['day_name']=pd.to_datetime(pos1['Date']).dt.day_name(locale='ko_kr')
...
q2_out=ttest_ind(q2[0].dropna(), q2[1].dropna(),
          equal_var=False)
...
# 2. 요일별 주력 상품의 판매 개수: 일자별로 주력상품 수를 산출, 요일별로 그룹
q3_2=q3.groupby(['Date', 'day_name']).apply(len).reset_index()

q3_2.columns=['Date', 'day_name', 'freq']
...
anova_out=ols('freq~day_name', q3_2).fit()

# p-value는 반올림하여 소수점 둘째 자리까지 기술
