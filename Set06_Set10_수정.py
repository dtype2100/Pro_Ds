...
# 절대값이 가장 큰 변수와 가장 작은 변수를 차례로 기술

q1.idxmax() # 'sqft_living'
q1.idxmin() # 'yr_built'
...
# 통계적 유의성을 갖지 못하는 독립변수를 제거하면 회귀 모형에 남는 변수는 모두
# 몇 개인가? 
...
(ols1.params[q3_out1.index] < 0).sum() # 2
...
# 1. GRE 점수의 평균 이상을 받은 그룹과 평균 미만을 받은 그룹으로 변수 생성
q2=data7.copy()
q2['GRE_gr']=np.where(q2.GRE >= q2.GRE.mean(), 1, 0)

# 2. 독립인 이표본 t 검정 : 양측 검정 (등분산으로 가정)
...
# 로지스틱 회귀계수의 절대값이 가장 큰 변수와 그 값을 기술
...
# 답: 'CGPA',    1.9785838109112257

# array 구조: numpy 함수 사용
...
# 답: (California, Florida New York) 0.34, 0.32, 0.34
...
# 1.나이는 20 이하이면 10, 30 이하이면 20, 40 이하이면 30, 50 이하이면 40, 60 이하이면 50, 
# 60 초과는 60으로 변환하여 Age_gr으로 파생변수 생성
...
#  연관성이 있는 것으로 파악된 변수의 검정통계량 추정치를 정수 부분
