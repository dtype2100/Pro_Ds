...
data11 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_11.csv')
data11.columns
...
# DF.apply(함수명 또는 Lambda ~ 함수 제작 가능)
...
from statsmodels.stats.multicomp import pairwise_tukeyhsd # 디중비교, 사후 검정
...
# 다중비교/사후검정: 귀무가설 기각 시 사용, 평규 차이나는 그룹 존재 시 사용
...
pos1 = pd.read_csv('C:/Users/user/OneDrive/바탕 화면/Pro_Ds/Dataset/Dataset_15_Mart_POS.csv')
list1 = pd.read_csv('C:/Users/user/OneDrive/바탕 화면/Pro_Ds/Dataset/Dataset_15_item_list.csv')
...
pos2=pos1.copy()
pos2['month'] = pd.to_datetime(pos1['Date']).dt.month #문자열 날짜를 날짜 데이터로 변경
pos2['day_name'] = pd.to_datetime(pos1['Date']).dt.day_name(locale='ko_kr')

pos2['week'] = np.where(pos2['day_name'].isin(['금용일', '토요일']), 1, 0)
