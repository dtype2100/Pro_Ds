...
data6 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_06.csv')
data6.columns
...
#q1=data6[var_list].corr()
q1=data6[var_list].corr().drop('price')['price'].abs()


q1.idxmax() # 답: 'sqft_living'
q1.idxmin() # 답: 'yr_built'
...
#통계적 유의성을 갖지 못하는 독립변수를 제거하면 회귀모형에 남는 변수는 모두
# 몇 개인가
...
data7 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_07.csv')
data7.columns
...
# GRE 점수의 평균 이상을 받은 그룹 과 평균 미만을 받은 그룹의 변수 생성
...
ttest_out = ttest_ind(q2[q2.GRE_gr == 1]['CGPA'],
          q2[q2.GRE_gr == 0]['CGPA'], # 결측치가 있는 경우 .dropna()
...
#  로지스틱 회귀분석 수행 결과에서 로지스틱 회귀계수의 절대값이 가장 큰 변수와 그 값을 기술
...
# array 구조: numpy 함수 사
var_list[abs(logit.coef_).argmax()]
abs(logit.coef_).max()

# 답: 1.9785838109112257
#%%
...
data8 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_08.csv')
data8.columns
...
#답: ekqCalifornia, Florida, New York: 0.34, 0.32, 0.34
...
q3_out.iloc[q3_out['mape'].argmin(),0]
q3_out.loc[q3_out['mape'].idxmin(), 'state']
q3_out[q3_out['mape'].argmin()]
...
import pandas as pd
import numpy as np
data9 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_09.csv')
data9.columns
...
#1.  결측값이 있다면 해당 행을 제거하시오. - 나이는 20 이하이면 10, 30 이하이면 20, 40 이하이면 30, 50 이하이면 40, 60 이하이면 50, 
...
# 3. 테스트 데이터로 f1 score 성능 평가

pred = logit.predict(test[var_list])

from sklearn.metrics import f1_score, calssification_report

f1_score(test['satisfaction'], pred, poslabel='dissatisfied')
...
data10 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_10.csv')
...
tab = q1_2.groupby('model')[['age_in_days', 'km']].mean() # 복수의 값을 선택할 때는 대괄호[] 2개 사용
tab['km_per_day'] = tab['km']/tab['age_in_days']



# 3. 가장 낮은 값을 가진 모델이 가장 큰 값을 가진 모델에 대한 비율
...
g_max=tab['km_per_day'].idxmax()
g_min=tab['km_per_day'].idxmin()

max_df = q2[q2.model == g_max]['km_per_day']
min_df = q2[q2.model == g_min]['km_per_day']
...
# 1.  독립변수로 engine_power, age_in_days, km를 사용하고 종속변수로 price
...
# 답:  array([10367.53433763]
 # 참고
test = pd.DataFrame()([[51, 400, 9500]], columns=var_list)
lm_pop.predict(test)
