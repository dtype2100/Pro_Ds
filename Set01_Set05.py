...
data1=pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_01.csv')
...
var_list = ['TV', 'Radio', 'Social_Media', 'Influencer', 'Sales']
...
from sklearn.linear_model import LinearRegression # array, 상수와 회귀게
from statsmodels.formula.api import ols #강추, 자동적으로 범주형변수를 더미로 전환, 분산분석 가능, pvalue 기반 의사결정 가능
from statsmodels.api import OLS, add_constant # 데이터 프레임으로 리턴

var_list = ['TV', 'Radio', 'Social_Media']

# ols(식, 데이터셋).fit()
# 종속변수(y) ~ 독립변수(x1+C(x2)+x3-1) '-1' 사용 시 상수항 제외
# -1 : 상수항 미포함
# C() : 범주형 변수로 인식

# ols('Sales~TV, Radio, Social_Media', data1).fit()

form1= 'Sales~' + '+'.join(var_list) #+ '-1' # -1의 여부에 따라 상수항 포함, 미포함
q3_df=data1.dropna()
q3=ols(form1, data1).fit()

dir(q3)
q3.summary()

q3_out = q3.params.drop('Intercept') #coef 값만 확인
...
data2=pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_02.csv')
data2.columns
...
# (답안 예시) 3, 1.23456
# =============================================================================
# 카이제곱: 적합성:변수(범주) 수 1, 독립성: 2, 동질성:2
# 관측(oij), 기대빈도(Eij)
...
# 3. Root Node에서 선택된 변수(split feature)와 분리 기준값 (split values)
...
data3=pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_03.csv')
...
q1_out = q1[(q1['forehead_ratio'] > UB) | (q1['forehead_ratio'] < LB)] # '|' -> or
...
# ttest_1samp: 1표번 t검정
# ttest_ind: 독립인 2표본 trjawjd, 등분산여부 반영
from scipy.stats import bartlett, levene #등분산 여부 검정
#ttest_rel: 대응인 2표본 t 검정

g_f = q1[q1.gender == 'Feamale']['forehead_ratio']
...
pred_log = logit.predict(test[var_list])
...
data4 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_04.csv')
data4.columns
...
# 2. 년도별 육류 소비량 합계
q1_df = q1.groupby('TIME').sum().reset_index()
...
# (2) 연도별로 데이터가 쌍이 되도롤 설계

temp=q2[q2.SUBJECT == 'BEEF']

temp = pd.pivot_table(q2, index= 'TIME',
                      columns= 'LOCATION',
                      values= 'Value',
                      aggfunc='mean').dropna()

# (3) 반복 적용
 #ttest_rel 만 사용할경 우 메모리 효율좋음
from scipy.stats import ttest_rel
...
for i in sub_list:
    ...
    temp2 = pd.pivot_table(q2, index= 'TIME',
                          columns= 'LOCATION',
                          values= 'Value',
                          aggfunc='mean').dropna()
...
# [참고]
#glovals()['변수명']=모델 => eval('문자열) -> 문자열을 오브젝트로 인식
#locals()
q3_out2=[]
for i in sub_list:
    temp=q3[q3.SUBJECT == i]
    globals()['lm_'+i]=LinearRegression().fit(temp[['TIME']], temp['Value'])
    pred=eval('lm'+i).predict(temp[['TIME']])
    r2_score=eval('lm'+i).score(temp[['TIME']], temp['Value'])
    # (MAPE = Σ ( | y - y ̂ | / y ) * 100/n 
    mape= (abs(temp['Value'] - pred) / temp['Value']).sum() * 100 / len(temp)
...
data5 = pd.read_csv('C:\\Users\\user\\OneDrive\\바탕 화면\\Pro_Ds\\Dataset\\Dataset_05.csv', na_values=['?', ' ', ''])
data5.columns
#['ID', 'Age', 'Age_gr', 'Gender', 'Work_Experience', 'Family_Size',
#      'Ever_Married', 'Graduated', 'Profession', 'Spending_Score', 'Var_1',
#     'Segmentation'],

# na_values (자동적으로 결측치로 데이터 로드)(' ')스페이스가 들어가있는 문자열 object. ('')값은 없지만 object라고 등록된 것이라 인식
# 스페이스가 들어가있는 경우, 문자열로 인식
...
# 2. Train대 Test 7대 3으로 데이터를 분리한다. (Seed = 123)
